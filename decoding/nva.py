...
def generate2(**kwargs):
    model, teacher_model, encoder_outputs, teacher_encoder_outputs, category, tgt_tokens, tgt_vocab, opt, dict_mapping, length_bias = map(
            lambda x: kwargs[x], 
            ["model", "teacher_model", "encoder_outputs", "teacher_encoder_outputs", "category", "tgt_tokens", "tgt_vocab", "opt", "dict_mapping", "length_bias"]
        )


    strategy = NVA(opt['iterations'], opt['seed'], dict_mapping=dict_mapping)
